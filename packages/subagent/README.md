...
Ignored dependency directories use copy-on-write copies when available. On macOS the runtime clones each dependency directory as one unit through `clonefile(2)`, which finishes in seconds where a file-by-file `cp -c` needs a minute for a large `node_modules`. The call goes through the `python3` that ships with the same Xcode Command Line Tools as `git`. When that is unavailable, or the volume rejects a directory clone, the runtime falls back to `cp -c`, then to a regular copy. Dependency materialization runs in the background after the workspace becomes usable, so the child starts before large copies finish. The product repository completes first and exposes its own barrier. The first tool call awaits only the product repository, while isolation capture and workspace cleanup await every repository. Repeated task caches under an ignored dependency directory bind to one shared store per repository, so every writer reuses the warm cache instead of copying it.
...
A capability can provide `terminalValidation: { maxCorrections, validate }`. The typed validator runs against immutable captured isolation before descendant success settlement or destination integration. Staged descendants may be merged only into an isolated parent preview; their receipts remain staged until validation succeeds. A scope conflict fails before validation. The validator receives the output artifact, structured output, captured isolation, workspace identity, read-only policy, current-attempt tool receipts, and prior terminal output revisions. A report-contract rejection can request a bounded follow-up in the same child session. A validator returns `correctionAllowed: false` for execution or artifact failures that report-only correction cannot fix. An accepting validator can return `normalizedOutput` to replace the terminal text with a deterministic repair, which spends no correction turn and re-resolves the structured output and gates. The runtime keeps the session tool set installed during an allowed follow-up and blocks every tool call in that turn, so the cached prompt prefix stays stable. The runtime publishes each raw output as a `TerminalOutputRevision` and fails settlement with the validator diagnostics when the bound is exhausted. Trusted `terminalFailureKind: "report-contract"` metadata is set only when allowed report correction is exhausted. It never redispatches the Task or creates another workspace.
