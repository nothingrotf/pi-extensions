...
test.each([
  ...
  [hudPath, filetoolsPath],
  [filetoolsPath, hudPath],
])(
  'loads filetools beside another tool provider without conflicts: %s first',
  async (first, second) => {
    ...
    try {
      ...
      if (first === tgrepPath || second === tgrepPath) expect(owners('grep')).toEqual([tgrepPath])
    } finally {
