...
The orchestrator passes this file to every parallel candidate runner during Phase B. It supplies the task, Phase A grounding artifacts, isolated working directory, and output path. Repository writers use a managed `Task` worktree. Other artifacts use a separate directory for each runner.
...
- Make boundaries visible. Use `not implemented` errors for bodies and separate pseudocode blocks for tricky logic. Put intent and invariants in the rationale. A reader should trace data from input to output through types and signatures alone.
...
You are one of several independently configured runners. Produce the best design you can make. Don't hedge against the others. Differences between candidates are the signal used to pick a base and graft. Converging on a safe-looking middle defeats the exploration.
