...
The why skill uses one source-specific playbook for each available evidence category. The parent reads the playbook and queries the active source tool. The playbooks are concrete examples for common MCPs. Adapt them for a different tool in the same category.
