...
Start by finding the relevant code. Use `find` to locate directories and files. Use `grep` to locate key symbols. Use `read` to inspect the implementation. Don't guess from names. Read the code.
