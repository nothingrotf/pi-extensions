...
- When you fan work out to subagents, write the lever as a skill they all read: the recipe, the verification contract, and the do-not-touch fences in one artifact, so every delegate inherits the same hardened version. Keep it outside the delegates' write scope so they can't quietly edit the contract.
