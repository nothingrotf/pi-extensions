...
const LANES = "Ten lanes on the configured `swarm workers` models at the PR head";
const SUB_BLOCKS = [
	"Depends on.",
	"Files.",
	"Build.",
...
const PROGRAM_MARKERS = ["/goal", "bundled", /30[- ]minute/, "status message"];
...
process.stdout.write(`${[...report, `${prSections.length} PR sections, ${problems.length} problems`].join("\n")}\n`);
