...
2. Pick the wake mechanism using the installed `loop` skill. For an observable event, use `loop_next` with `watch.command` and a long heartbeat fallback. Without an event, select a fixed interval for the next useful check. Wakeups stay in the current persistent session, not a Task child. If Loop is unavailable, report that blocker instead of inventing a watcher.
