...
3. Plan the fix from the trace. If it crosses a function boundary, `architect` first. Delegate implementation to a subagent using the configured `perf-issue` model and `role: "perf-issue"`. Follow `references/task-contracts.md` for capabilities and isolation. Concrete selectors use `provider/model-id:effort [fast]`. Omit `Task.model` to use the scalar runtime policy. Pass `capability_profile: "pstack-leaf"` for a leaf or `pstack-nested` for a delegating owner. Review the diff. Capture a post-fix trace.
