...
3. Plan the fix from the trace. If it crosses a function boundary, `architect` first. Delegate implementation to a subagent using the configured `perf-issue` model. Concrete selectors use `provider/model-id:effort [fast]`. Omit `Task.model` for an absent role, `auto`, or `inherit-parent`. Review the diff. Capture a post-fix trace.
