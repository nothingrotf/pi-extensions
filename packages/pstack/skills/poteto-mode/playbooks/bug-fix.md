...
3. Plan the fix. If it crosses a function boundary, `architect` first. Delegate implementation to a subagent using the configured `bug-fix` model and a specific scope. Concrete selectors use `provider/model-id:effort [fast]`. Omit `Task.model` for an absent role, `auto`, or `inherit-parent`. Review the diff.
