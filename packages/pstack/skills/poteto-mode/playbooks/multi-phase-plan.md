...
3. Explore in subagents with `subagent_type: "poteto-agent"` and the model contract from the Subagents section (the **guard-the-context-window** principle skill). Each returns file pointers, conventions, test commands, and entry points. No inlined dumps.
...
Each live lane runs in its own isolated `Task` worktree at the PR head. Drive through `control-ui` or `control-cli`.
