...
1. Locate the prior trail through `session_history`, a retained `Task` run reference, a resume note, or a pushed branch. Read the metadata and last messages first, then scan back for decision points. Do not search unrelated sessions. Parse a long transcript in a read-only subagent and keep the reduced timeline in the main thread (the **principle-guard-the-context-window** skill).
