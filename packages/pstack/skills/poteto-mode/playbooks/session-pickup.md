...
**You own the resume point. Read the prior trail, don't redo it.** For "take over this", "resume this conversation", "continue from <transcript path>", "you're taking over", "pick up where X left off", a retained run reference, or a pushed branch you're meant to continue.

1. Locate the prior trail through `session_history`, a retained `Task` run reference, a resume note, or a pushed branch. Read the metadata and last messages first, then scan back for decision points. Do not search unrelated sessions. Parse a long transcript in a read-only subagent with `role: "judgment and prose"` and `capability_profile: "pstack-leaf"` and keep the reduced timeline in the main thread (the **principle-guard-the-context-window** skill).
