...
- Spawns and resumes reference the standing-orders file and carry the bounded constraints relevant to their scope.
...
- **Worker / verifier.** Use `subagent_type: "poteto-agent"` for code workers and ad-hoc helpers. Give `capability_profile: "pstack-nested"` only to owners that must delegate. Leaf `poteto-agent` workers use the default `pstack-leaf` profile. Set `Task.role` to an exact policy role on every spawn. Implementation uses `feature`, `bug-fix`, `refactoring`, `perf-issue`, or `hillclimb`. Track owners use `hardest tasks` with `pstack-nested`. Stacker work uses `refactoring` with `pstack-leaf`. Static review and audit use `code review`. Executable acceptance checks and babysit verification use `runtime verification`. Destination Git and PR operations use `publication`. Evidence and prose synthesis use `judgment and prose`. These leaves use `pstack-leaf`. Select one permitted model for each reviewer or verifier, not every entry in its pool. Use `Task` with a managed worktree for each repository writer. Use read-only mode for static verifiers. A runtime verifier that needs `control-ui` or `control-cli` runs as mutable background work with `isolation: { mode: "worktree", integration: "manual" }`. Never join its incidental patch. Use `session_history` only with known session references. Briefs inline bounded facts or point at repository paths. Prefer fewer, broader workers. Keep one writer per worktree or branch (principle-separate-before-serializing-shared-state). Use a different configured model family for the verifier when available.
...
- `preferences.md` is the standing-orders register. Use numbered lines with one constraint each. Include model policy, Task routing, stack shape, verification, forbidden paths, and escalation. Record `poteto-agent` and `pstack-nested` rules exactly. Reference its absolute path on spawns and resumes. Include its identity and the constraints relevant to that unit without duplicating unrelated standing orders. Directives decay across resumes, and each dropped directive costs a human turn. If you restate an instruction, append the instruction before you act (principle-encode-lessons-in-structure).
...
CONTEXT      immutable pointers to files, PRs, and upstream reports; inline only the
             bounded evidence this unit needs when those references are inaccessible
ACCEPTANCE   checkable criteria, one per line
VERIFY       exact commands or the control-skill path, plus known gotchas
TIMEBOX      diagnostic checkpoint; stop only for an explicit operator limit or genuine blocker
FORBIDDEN    no stack backend commands, no rebase, no force-push, no fixes outside scope, plus unit-specific bans
REPORT       status, branch, head SHA, PRs, verdict, what you actually ran, deviations,
             suggested follow-ups
STANDING     <preferences.md reference and identity, plus relevant constraints>
```

Size the brief to the unit. A one-command unit gets the template collapsed to a paragraph that still names goal, scope, the verify command, and the report shape. A 4KB scaffold around a two-line edit costs more to write and obey than the edit. Spawns and resumes reference the standing-orders file by store path and identity. Carry only relevant constraints inline. Carry the per-issue fields from `../references/delivery-contract.md` in the brief: issue, phase, accepted design and evidence refs, base and head identity, verification owner, and acceptance criteria. A leaf implements the accepted design it receives, so an issue that merely crosses a function does not return for design. Read `references/task-contracts.md` before dispatch. Never instruct an isolated worker to edit, commit, or push from the source checkout. Accept its captured patch first. Then delegate the destination commit and push as a separately scoped foreground operation. Do not push synthetic snapshot history as the product branch.
...
A dependency is a context relay, not just ordering. Undeclared upstream context makes the worker guess. Missing fields are a refuse-to-spawn condition. Audit one sampled worker brief per sub-coordinator per wave, concurrently with the wave it samples, never as a gate in front of it. A failing brief stops that track and fixes the sub-coordinator's instructions, not just the worker, because brief quality decays late in a run. Resume the same implementer and independent reviewer through compatible corrections. Preserve their role, capabilities, accepted design, and artifact access. Start a fresh child with consolidated scope when its contract becomes incompatible or a diagnosed context failure requires replacement. Carry forward the harness, findings, and evidence instead of repeating discovery.
...
2. **Install the runtime.** Run `orch init`. Open the trail via the show-me-your-work skill. Write the standing orders before any spawn. Select one backend through `../references/stack-backends.md`. Seed `frontier.json` from existing PRs with `orch frontier set --repo <repo-dir>`.
...
Follow `../references/delivery-operations.md` for compact checkpoints and stalled-correction diagnosis.
When no independent work remains, use `TaskControl.wait` with the typed example in `../references/task-contracts.md`.
Never substitute shell `sleep` or wait for a heartbeat after a child completes.
...
- The frontier is a computed object, never narrative. Recompute `frontier.json` through the selected backend after every merge and stack mutation: ordered PR list, branch names, head SHAs, a generation number, the lowest unmerged PR. Resolve it where that backend tracks the stack, normally the stacker's clone. A checkout without the selected backend's metadata must error rather than guess.
- Exactly one stacker per stack can run stack backend commands, serialized within its stack. Record the holder in the standing orders. Run rebases in one dedicated `Task` worktree so topology remains isolated.
- Workers never rebase and never run stack backend commands. Babysitters follow `playbooks/babysit.md`, one per stack, scoped to one immutable frontier generation. They report conflicts to the stacker rather than rebasing.
...
Scale verification to the unit. When VERIFY is a single cheap command, the worker runs it and reports the output, and the coordinator spot-checks receipts. A dedicated verifier agent (on a different model family than the worker) is for units whose verification is expensive, judgment-laden, or high-blast-radius. A verifier agent whose entire product would be rerunning one command is ceremony, not verification. Default to one independent reviewer per issue that never wrote the code, combining comment-cleanup recommendations, deslop, and static audit in one complete verdict when those passes are safe together. The implementer applies corrections. If runtime verification is expected, start that reviewer with `runtime verification`, shell access, and manual isolation. Do not change its role on resume. Corrections resume the same implementer while the accepted design stays compatible, and a correction rechecks only the affected behavior before the full required gates run. Lane count follows disjoint workstreams, never the size of a configured model pool.

Write ledger rows with `orch ledger record`. Check the current PR and head SHA with `orch ledger check`. `ledger.tsv`, one row per verdict, keyed by PR number plus head SHA: `live-ui-verified | unit-test-verified | type-check-only | verifier-blocked | verifier-failed`. CI green is an input to a verdict, not a verdict. A passing criterion matrix is required before promoting WIP to a verification candidate. Behavioral work needs better than `type-check-only`. `verifier-blocked` is not a pass. Respawn when the environment heals. `verifier-failed` gets a fix unit, not a re-verify. A worker may self-report. A verifier overrides it on the same key. A new head SHA voids the row, so re-verify after restack. The ledger answers "was this verified", not memory and not the transcript.
...
- Never resume an agent to check on it. A resume restarts an idle agent. Probe read-only: the ledger, `units.tsv`, `gh`, pushed branches, retained `Task` status and completion evidence. Transcript mtime is not liveness.
...
- Diagnose before retrying: inspect cap or memory failures, reconcile uncertain network outcomes, and correct tool arguments from their descriptors. Two incomplete returns without meaningful progress require the delivery operations diagnosis. Do not change models for a schema typo or abandon authorized scope silently.
...
- After a Pi restart, re-read the standing orders and `units.tsv`. Recompute the frontier. Reconcile retained runs by PR, branch, and run reference. Respawn missing sub-coordinators from stored briefs and current state. Then drain and resume.
