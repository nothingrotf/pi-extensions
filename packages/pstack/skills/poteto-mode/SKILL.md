...
compatibility: Requires Git and Bun. PR workflows require gh. Stack workflows require gt or github/gh-stack. Worktree audits require jq.
---
...
**Start every multi-step task with `todo_write`. Make its first item read the Principles section below in full.** The principles ground every trigger here. In your reply, name each principle that shaped a decision and the specific choice it changed. A citation with no decision behind it means you skipped its leaf skill; it must trace to a real choice the leaf's rule drove.
...
- Shipping UI / IDE / CLI → the matching control skill. Use **control-cli** for CLIs and TUIs. Use **control-ui** for browser, Electron, and web UIs. For bug fixes, reproduce first on the same surface yourself. Ask the user only when no available tool can reach it.
- Any PR-status request → the **Babysit** playbook (`playbooks/babysit.md`). That includes "babysit this", "get it green", "address the bugbot comments", and the commonest phrasing, "check on PR X" / "anything outstanding on X". Never triggered by merely opening a PR. Declare its mode before polling; the playbook's step 1 owns the request-to-mode mapping. Reaching for `drive` inside a phase agent stops that agent finishing its turn.
- Asked to land or ship a green stack → the **Shipping** playbook (`playbooks/shipping.md`). Green is not safe. Nothing gets armed or merged before an independent per-PR verdict, and only the contiguous verified run from the root lands.
...
The `poteto-agent` defaults to background mode. Set `run_in_background: false` when its result blocks the current step. A mutable background Task receives an isolated worktree automatically. Its changes stay staged after completion. Use relative product paths inside the effective child workspace. Never bypass isolation through an absolute source checkout path. Never push synthetic snapshot history as a product branch. Inspect the terminal result, then call `TaskControl` with `action: "join"` only after acceptance. Cancel an isolated writer for a zero-write stop. Use `TaskControl` status, list, steer, and cancel actions instead of a resume for active work. Background results arrive as follow-up messages. Do not poll. If you are blocked with no other work, call `TaskControl` with `action: "wait"`.
...
Concrete configured models use `provider/model-id:effort [fast]`. Omit `Task.model` for an absent role, `auto`, or `inherit-parent`. Never invent a model selector.
...
Do not write code comments. Use names, types, assertions, and structure to make intent visible. This rule applies to every generated file and every delegated diff.

## Playbooks

Your first `todo_write` items are the matched playbook's steps, copied verbatim, before any task-specific todos and before you reason about the task. The failure mode is reading a playbook then writing a bespoke plan that drops its named steps (`architect`, the throughput checkpoint). A step you choose not to do stays in the list with a one-line `skip: <reason>`; skipping silently is not allowed. Match the task to a playbook below, open its file, and copy its steps in verbatim.
