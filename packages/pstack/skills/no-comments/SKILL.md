...
1. Spawn `Task` with `subagent_type: "Comment Sicko"`. Pass `role: "judgment and prose"` and `capability_profile: "pstack-leaf"`. Pass the scope. Do not restate its rules. Follow `../poteto-mode/references/task-contracts.md` for managed writer isolation and diff acceptance.
