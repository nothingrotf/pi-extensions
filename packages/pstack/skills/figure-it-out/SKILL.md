...
Use `todo_write`. Its first item reads the full **poteto-mode** skill, including its Principles and Subagents contracts. Then add the phases below as todos.
...
- For one-way-door design decisions, run the **architect** skill (it runs **arena**) with diverse, isolated, opinionated candidates and a read-only judge on a verified different model family. If no different-family model is available, return `BLOCKED`. Do not substitute a model. Skip it for mechanical work whose shape is already concrete. A second arena over a settled design is over-engineering (the **laziness-protocol** principle skill).
...
Then put the design into motion. Add its steps to the `todo_write` list as concrete items, after the Phase C entry and before Phase D. Run each under the Phase C loop discipline, and weave the Phase D log through them, a row as each step lands, rather than saving the whole trail for the end.
...
Log the run via the **show-me-your-work** skill, with one row per decision and unit. Let that skill own the trail format and final different-family review. figure-it-out's work is usually ambitious enough to commit the trail so the reviewer can read it in the PR; commit it when confidence has to be shown. Prefer evidence produced by committed scripts so a reviewer can re-run it. The trail plus the diff is what lets the human come back and trust the work.
