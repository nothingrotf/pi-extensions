...
Before handoff, assign the trail audit to an independent reviewer from a different model family than the implementation. Reuse the existing independent reviewer when its contract and evidence access remain compatible. Include the trail audit in its complete verdict rather than spawning a separate reviewer for the same evidence. If no compatible reviewer exists, spawn one read-only `Task`. Pass `subagent_type: "generalPurpose"`, `role: "code review"`, and `capability_profile: "pstack-leaf"`. Follow [Task contracts](../poteto-mode/references/task-contracts.md). The different-family requirement does not override the configured model policy. Select only a permitted model that the Pi runtime verifies. If the policy or available models prevent a different-family review, return `BLOCKED` and request a policy update. Self-review is not a substitute. The parent owns `session_history`, because a `Task` child does not inherit extension tools. Pass the log path, bounded timeline results, bounded tool activity results, and their stable references to the subagent. The subagent reads that evidence, then flags what the user must examine. This review scans for suboptimal or risky work. It does not redo the work.
