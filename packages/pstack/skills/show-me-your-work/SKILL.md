...
Before handoff, use `Task` to spawn a read-only subagent on a different model family from the one that did the work. Pass `subagent_type: "generalPurpose"`, `role: "judgment and prose"`, and `capability_profile: "pstack-leaf"`. Follow [Task contracts](../poteto-mode/references/task-contracts.md). The different-family requirement still overrides an inherited or same-family model policy. Select only a model that the Pi runtime verifies. If no different model family is available, return `BLOCKED` and name the missing capability. Self-review is not a substitute. The parent owns `session_history`, because a `Task` child does not inherit extension tools. Pass the log path, bounded timeline results, bounded tool activity results, and their stable references to the subagent. The subagent reads that evidence, then flags what the user must examine. This review scans for suboptimal or risky work. It does not redo the work.
